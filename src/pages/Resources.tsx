...
const Resources = () => {
  ...
  queryKey: ['resources'],
  queryFn: async () => {
    // 1. Fetch from external Supabase (supabase2)
    const { data: externalResources, error: externalError } = await supabase2
      .from('resource_requests')
      .select('*');

    if (externalError) throw externalError;

    // 2. Transform and insert into your primary Supabase
    const { error: insertError } = await supabase
      ...
      .upsert(
        ...
        { onConflict: 'id' } // Update existing if ID matches
      );

    if (insertError) throw insertError;

    // 3. Fetch combined data from your primary Supabase
    const { data: localResources, error: localError } = await supabase
      .from('resources')
      .select(`
        *,
